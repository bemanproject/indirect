...
**Implements**: `std::identity` proposed in [Standard Library Concepts (P3019)](https://wg21.link/P3019).
...
The following code snippet illustrates how we can achieve a default projection using `beman::indirect::identity`:
...
[`BEMAN_INDIRECT_BUILD_TESTS`](#beman_indirect_build_tests) to `OFF`
...
[![Open in GitHub Codespaces](https://github.com/codespaces/badge.svg)](https://codespaces.new/bemanproject/indirect)
...
Project-specific options are prefixed with `BEMAN_INDIRECT`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_INDIRECT" -C 2
```
...
```bash
cmake -B build -S . -DCMAKE_CXX_STANDARD=20 -DBEMAN_INDIRECT_BUILD_TESTS=OFF
```

> [!TIP]
> Because this project requires GoogleTest for running tests,
> disabling `BEMAN_INDIRECT_BUILD_TESTS` avoids the project from
...
#### `BEMAN_INDIRECT_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: ON.
Values: { ON, OFF }.

This is required so that users of `beman.indirect` can use
`find_package(beman.indirect)` to locate the library.
...
The process for incorporating `beman.indirect` into your project depends on the
build system being used. Instructions for CMake are provided in following sections.

### Incorporating `beman.indirect` into your project with CMake

For CMake based projects,
you will need to use the `beman.indirect` CMake module
to define the `beman::indirect` CMake target:

```cmake
find_package(beman.indirect REQUIRED)
```

You will also need to add `beman::indirect` to the link libraries of
any libraries or executables that include `beman.indirect` headers.

```cmake
target_link_libraries(yourlib PUBLIC beman::indirect)
```

### Produce beman.indirect static library

You can include indirect's headers locally
by producing a static `libbeman.indirect.a` library.
...
└── lib
    ...
    │       ├── beman.indirect-targets-debug.cmake
